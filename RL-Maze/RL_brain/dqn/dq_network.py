...
class DeepQNetwork:
    ...
    def learn(self, observation_, reward_table):
        ...
        [loss, accuracy] = self.eval_model.train_on_batch(batch_memory.iloc[:, :self.params['n_features']], q_target)
        ...
        if self.double_q:                       # 记录DoubleDQN的误差
            self.collections.add_double_loss(loss, accuracy)
        else:                                   # 记录DQN的误差
            self.collections.add_dqn_loss(loss, accuracy)

...
"""
    ...
    （2）Loss，accuracy曲线横向对比：（相同迷宫 不同算法）
...
class Collect:
    ...
    def loss_acc_f1_compared(self):
        ...
        ax_loss.legend()
        # 轴名称，刻度值的颜色
        ax_loss.axis['left'].label.set_color(ax_loss.get_color())
    ...
    def loss_acc_compared(self):
        ...
        ax2 = ax.twinx()
        ax.plot(np.arange(len(self.dqn_loss)), self.dqn_loss, color='#cb33ff', label='DQN_loss', linewidth='1.2', linestyle='-')
        ax2.plot(np.arange(len(self.dqn_acc)), self.dqn_acc, color='#f3ccff', label='DQN_acc', linewidth='1', linestyle=':', marker='>')
        ax.plot(np.arange(len(self.double_loss)), self.double_loss, color='#ff9933', label='DDQN_loss', linewidth='1.2', linestyle='-')
    ...
    def loss_compared(self):
        ...
        plt.plot(np.arange(len(self.dqn_loss)), self.dqn_loss, color='#cb33ff', label='DQN', linewidth='1', linestyle='-')
        plt.plot(np.arange(len(self.double_loss)), self.double_loss, color='#ff9933', label='DDQN', linewidth='1', linestyle='-')
        plt.legend()  # 显示图例说明Label标签
    ...
    def figure_different_scores_steps_compared(self):
        ...
        if len(self.q_step) > 0 or len(self.s_step) > 0 \
            ...
            plt.figure("Different Algorithm In Current Maze", figsize=(18, 9))
            plt.subplot(2, 1, 1)
            self.steps_compared()   # 不同算法 步长对比
            plt.subplot(2, 1, 2)
            self.scores_compared()  # 不同算法 得分对比
            now = time.strftime("%Y%m%d%H%M%S", time.localtime(time.time()))
            plt.savefig('./data_analysis/dif_scores_steps/'+self.map_name+'-'+now+'.png')
        else:
